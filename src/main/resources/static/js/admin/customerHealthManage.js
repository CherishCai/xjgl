	...
	$(document).ready(function() {
		oTable = $('#otable').DataTable(
			...
			"ajax" : function(data, callback, settings) {
				//封装请求参数
				var param = customerHealthManage.getQueryCondition(data);
				$.ajax({
					type : "GET",
					url : "/customerHealth/page",//TODO
					cache : false, //禁用缓存
					data : param, //传入已封装的参数
					dataType : "json",
			...
			"columns" : [
			    CONSTANT.DATA_TABLES.COLUMN.NO,
			    {
                    ...
                    "render": function (data, type, row, meta) {
                        ...
                        console.log("row:"+row);
                        var btn_group = "<a href='/customerHealth/detail/"+data+"' class='op_info btn btn-info' role='button'>详情</a>";
                        return btn_group;
                    }
                }
				],
	...
	var customerHealthManage = {
		...
		getQueryCondition : function(data) {
			...
			//组装查询参数
			param.fuzzySearch = customerHealthManage.fuzzySearch;
			if (customerHealthManage.fuzzySearch) {//模糊查询
