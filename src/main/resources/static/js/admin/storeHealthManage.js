	...
	$(document).ready(function() {
		oTable = $('#otable').DataTable(
			...
			"ajax" : function(data, callback, settings) {
				//封装请求参数
				var param = storeHealthManage.getQueryCondition(data);
				$.ajax({
					type : "GET",
					url : "/storeHealth/page",//TODO
					cache : false, //禁用缓存
					data : param, //传入已封装的参数
					dataType : "json",
	...
	var storeHealthManage = {
		...
		getQueryCondition : function(data) {
			...
			//组装查询参数
			param.fuzzySearch = storeHealthManage.fuzzySearch;
			if (storeHealthManage.fuzzySearch) {//模糊查询
				param.fuzzy = $("#fuzzy-search").val();
			} else {//非模糊查询
                var healthId = $("#healthId").val();
                if ("0" != healthId ){
                    param.healthId = $("#healthId").val();
                }
